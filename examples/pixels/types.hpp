...
//@abi table pixels i64  主键为行号
struct pixel_row {
    uint64_t row;
    std::vector<pixel> pixels; // 这行的所有像素数组
...
uint64_t calculateWithdrawalAndUpdate(const canvas &cnv, account &player,
                                      guard &grd) {
    int64_t patronBonus = cnv.patronBonusScaled(player) / PRECISION_BASE; // 获取用户分红

    int64_t balance = player.balanceScaled / PRECISION_BASE;

    int64_t withdrawAmount = patronBonus + balance; // 可提现金额
    ...
    if (withdrawnBalanceScaled >= player.balanceScaled) { // 防止精度问题导致用户余额没有完全归零
        player.balanceScaled = 0;
    } else {
        player.balanceScaled -= withdrawnBalanceScaled;
    }
    player.maskScaled += withdrawnBonusScaled; // 更新用户的已提金额
...
struct st_pixelOrder {
    uint32_t coordinate; // 包含坐标的一个值。 每行像素个数的整除值就是第几行，每行像素个数的求余值就是第几列
    uint32_t color;
    uint8_t priceCounter;
...
struct st_transferContext {
    ...
    st_buyPixel_result purchase(const pixel &pixel,
        ...
        result.isFirstBuyer = isBlank; // 如果这个像素点是空白的，则是第一次购买
        ...
        eosio_assert(amountLeft >= nextPrice, "insufficient fund to buy pixel"); // 要求这次下单剩下的钱大于这个像素需要的钱

        if (isBlank) {
            // buying blank. The fee is the entire buy price.
            result.feeScaled = nextPrice * PRECISION_BASE; // 如果是新的像素，则整个价格充当手续费
        } else {
        ...
        amountLeft -= nextPrice; // 更新这次下单所剩下的余额
        paintedPixelCount++; // 更新这笔下单中已经购买成功的像素点数量
        totalFeesScaled += result.feeScaled; // 这笔下单的总fee
    ...
    void updateCanvas(canvas &cv) {
        ...
        bonusPerPixelScaled = patronBonusScaled / cv.pixelsDrawn; // 这次购买中 每个像素的分红
        eosio_assert(bonusPerPixelScaled > 0, "bonus is 0");

        cv.maskScaled += bonusPerPixelScaled; // 更新画布中的 每个像素的分红
    ...
    void updatePurchaserAccount(account &acct) {
        ...
        uint128_t maskUpdate =
                (canvasMaskScaled - bonusPerPixelScaled) * paintedPixelCount; // 除去自己的每个像素的分红 * 自己购买的像素个数
        acct.maskScaled += maskUpdate; // 更新已提分红
