...
void apply_context::exec_one( action_trace& trace )
{
   ...
   try {
      try {
         ...
         if( native ) {
            if( trx_context.enforce_whiteblacklist && control.is_producing_block() ) { // 如果本节点正在生产块
...
void apply_context::exec( action_trace& trace )
{
   ...
   exec_one( trace ); // 这一次执行，receiver是action的账户。这次执行会把require_recipient方法执行掉，通知器会被加入账户，下面就会通知他们。内联交易也会加到 _inline_actions 中
   for( uint32_t i = 1; i < _notified.size(); ++i ) {
      receiver = _notified[i];
      trace.inline_traces.emplace_back( );
      exec_one( trace.inline_traces.back() ); // 其他通知器中的账户，receiver是通知器中的账户，且结果属于inline_traces
   }
   ...
   if( _cfa_inline_actions.size() > 0 || _inline_actions.size() > 0 ) {  // 要求内联交易不能太多
      EOS_ASSERT( recurse_depth < control.get_global_properties().configuration.max_inline_action_depth,
                  transaction_exception, "max inline action depth per transaction reached" ); // recurse_depth 默认就是0，后面执行内联交易会递增
   }
